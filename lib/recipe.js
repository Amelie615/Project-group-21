...
function createRecipe(hashedTable, keysToHashed) {
    ...
    while (done === true) {
        ...
        var integersFromMeasurements = Math.floor(inputMeasurements.match(/(\d+)/)[1]); //asså det här är ju bara inte rätt men inte fel, lös det
        var lettersFromMeasurements = inputMeasurements.replace(/\d+|^\s+|\s+$/g, ''); //ful default
